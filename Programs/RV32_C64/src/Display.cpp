...
Display::Display(C64 * c64) : the_c64(c64)
{
	...
	bool ok;
	DisplayInfo di = get_display_info(&ok);
	if (!ok) {
		error_and_quit("fatal: SYS_GET_DISPLAY_INFO failed\n");
	}
	if (di.format != FORMAT_RGBA8888) {
		error_and_quit("fatal: unsupported format %ul (expected RGBA8888=%ul)\n", di.format, FORMAT_RGBA8888);
	}
	if (di.width != DISPLAY_X || di.height != DISPLAY_Y) {
		error_and_quit("fatal: display is %ux%u but program expects %dx%d\n",
				di.width, di.height, DISPLAY_X, DISPLAY_Y);
	}
	...
	texture_buffer = new uint32_t[DISPLAY_X * DISPLAY_Y];
	memset(texture_buffer, 0, DISPLAY_X * DISPLAY_Y * sizeof(uint32_t));
...
void Display::error_and_quit(const char * fmt, ...) const
{
	va_list args;
	va_start(args, fmt);
...
void Display::ShowNotification(std::string s)
{
	...
	notes[next_note].time = get_us(nullptr);
...
void Display::draw_overlays()
{
	...
	do {
		if (notes[i].active) {
			int elapsed_ms = (now - notes[i].time) / 1000;
...
void Display::Update()
{
	...
	uint32_t now = get_us(nullptr);
	if (now - last_pulse_us >= PULSE_ms * 1000) {
		pulse_handler();
		last_pulse_us = now;
	}
	...
	for (unsigned y = 0; y < DISPLAY_Y; ++y) {
		for (unsigned x = 0; x < DISPLAY_X; ++x) {
			outPixel[x + y * DISPLAY_X] = palette[inPixel[x + y * DISPLAY_X]];
		}
	}

	show_framebuffer(outPixel);
}
...
void Display::PollKeyboard(uint8_t *key_matrix, uint8_t *rev_matrix, uint8_t *joystick)
{
	while (key_available()) {
		...
		if (!(key & 0x8000)) {
			switch (key) {
				case SDL_SCANCODE_F9:	// F9: Quit
					the_c64->RequestQuit();
					break;

				case SDL_SCANCODE_F10:	// F10: Prefs (no UI on RV32, ignored)
				...
				case SDL_SCANCODE_F12:	// F12: Reset (hold Shift to clear memory, Ctrl to auto-start)
					if (is_key_down(SDL_SCANCODE_LCTRL)) {
						the_c64->ResetAndAutoStart();
					} else {
						the_c64->Reset(is_key_down(SDL_SCANCODE_LSHIFT));
					}
					break;
...
#undef DEBUG
				...
				default:
					translate_key(key, false, key_matrix, rev_matrix, joystick);
					break;
			}
		} else {
			key = SDL_Scancode(key ^ 0x8000);
			if (key == SDL_SCANCODE_KP_PLUS) {
				...
				translate_key(key, true, key_matrix, rev_matrix, joystick);
			}
		}

	}

	// Poll controller button → C64 keyboard mapping (controller 0 only)
	// D-pad/stick/fire are handled in C64::poll_joystick(); this covers custom key bindings
	if (!button_mapping.empty()) {
		for (const auto & [btn, keycode] : button_mapping) {
			bool down = is_controller_button_down(0, (SDL_GameControllerButton)btn, nullptr);
			if (keycode < 64) {
				int row = keycode >> 3;
				int col = keycode & 7;
				if (down) {
					key_matrix[row] &= ~(1 << col);
					rev_matrix[col] &= ~(1 << row);
				} else {
					key_matrix[row] |= (1 << col);
					rev_matrix[col] |= (1 << row);
				}
			} else if (keycode == KEYCODE_PLAY_ON_TAPE) {
				the_c64->SetTapeControllerButton(down);
			}
		}
	}
}
...
void Display::init_colors(int palette_prefs)
{
	...
	for (unsigned i = 0; i < 16; ++i) {
		palette[i] = (0xFF << 24) | ((uint32_t)b[i] << 16) | ((uint32_t)g[i] << 8) | r[i];
	}

	// Extra colors for UI elements  (ABGR order: A=0xFF, B, G, R in uint32 on little-endian)
	palette[fill_gray]   = 0xFF000000u | (0xd0 << 16) | (0xd0 << 8) | 0xd0;
	palette[shine_gray]  = 0xFF000000u | (0xf0 << 16) | (0xf0 << 8) | 0xf0;
	palette[shadow_gray] = 0xFF000000u | (0x40 << 16) | (0x40 << 8) | 0x40;
	palette[red]         = 0xFF000000u | (0x00 << 16) | (0x00 << 8) | 0xf0;
	palette[dark_red]    = 0xFF000000u | (0x00 << 16) | (0x00 << 8) | 0x30;
	palette[green]       = 0xFF000000u | (0x00 << 16) | (0xc0 << 8) | 0x00;
}
