...
constexpr size_t REWIND_LENGTH = 0;  // Disabled: 1500 × 68KB = ~100MB, exceeds 32MB RAM
...
C64::C64() : quit_requested(false), prefs_editor_requested(false), load_snapshot_requested(false)
{
	...
	// Allocate buffer for rewinding (REWIND_LENGTH = 0 disables rewind)
	rewind_buffer = (REWIND_LENGTH > 0) ? new Snapshot[REWIND_LENGTH] : nullptr;
}
...
void C64::vblank()
{
	...
	uint32_t now = get_us(nullptr);

	int elapsed_us = (int)(now - frame_start);
	...
	if ((elapsed_us < FRAME_TIME_us) && ThePrefs.LimitSpeed) {
		uint32_t remaining = frame_start + FRAME_TIME_us - now;
		if ((int32_t)remaining > 0) sleep_us(remaining);
		if (play_mode == PlayMode::Forward) {
			frame_start += FRAME_TIME_us / FORWARD_SCALE;
			frame_skip_factor = FORWARD_SCALE;
		} else {
			frame_start += FRAME_TIME_us;
		...
		// Dynamic frameskip: target skip = ceil(100 / speed_index)
		// At 100%+: skip proportionally to not exceed 50fps
		// At 80%: skip=2, at 50%: skip=2, at 33%: skip=4, etc.
		constexpr int MAX_FRAME_SKIP = 4;
		int target_skip;
		if (speed_index >= 100) {
			target_skip = speed_index / 100;
		} else {
			target_skip = std::min(MAX_FRAME_SKIP,
				(100 + speed_index - 1) / std::max(speed_index, 1));
		}

		// Smooth transitions: change by at most 1 per frame
		if (target_skip > frame_skip_factor)
			++frame_skip_factor;
		else if (target_skip < frame_skip_factor)
			--frame_skip_factor;
		if (frame_skip_factor < 1) frame_skip_factor = 1;

		if (frame_skip_counter > frame_skip_factor)
			frame_skip_counter = frame_skip_factor;
	}
	...
	while (true) {
		...
		if (ThePrefs.LimitSpeed && play_mode == PlayMode::Play) {
			...
			if (raster_y != prev_raster_y) {
				// frame_start points to end of current frame; sleep until 1/4, 2/4, 3/4 through it
				if (raster_y == TOTAL_RASTERS * 1 / 4) {
					uint32_t target = frame_start - FRAME_TIME_us * 3 / 4;
					uint32_t n = get_us(nullptr);
					if ((int32_t)(target - n) > 0) sleep_us(target - n);
					poll_input();
				} else if (raster_y == TOTAL_RASTERS * 2 / 4) {
					uint32_t target = frame_start - FRAME_TIME_us * 2 / 4;
					uint32_t n = get_us(nullptr);
					if ((int32_t)(target - n) > 0) sleep_us(target - n);
					poll_input();
				} else if (raster_y == TOTAL_RASTERS * 3 / 4) {
					uint32_t target = frame_start - FRAME_TIME_us * 1 / 4;
					uint32_t n = get_us(nullptr);
					if ((int32_t)(target - n) > 0) sleep_us(target - n);
					poll_input();
				}
...
{
	// Poll joysticks
	// Controller slot 0 maps to C64 port 2 (CIA Port A) by default — most games use port 2
	// Controller slot 1 maps to C64 port 1 (CIA Port B)
	TheCIA1->Joystick2 = poll_joystick(0);
	TheCIA1->Joystick1 = poll_joystick(1);
...
uint8_t C64::poll_joystick(int port)
{
	...
	// D-pad
	if (is_controller_button_down(port, SDL_CONTROLLER_BUTTON_DPAD_LEFT,  nullptr)) j &= 0xfb;
	if (is_controller_button_down(port, SDL_CONTROLLER_BUTTON_DPAD_RIGHT, nullptr)) j &= 0xf7;
	if (is_controller_button_down(port, SDL_CONTROLLER_BUTTON_DPAD_UP,    nullptr)) j &= 0xfe;
	if (is_controller_button_down(port, SDL_CONTROLLER_BUTTON_DPAD_DOWN,  nullptr)) j &= 0xfd;
	if (is_controller_button_down(port, SDL_CONTROLLER_BUTTON_A,          nullptr)) j &= 0xef;

	// Left trigger → rewind
	int trigger = get_controller_axis(port, SDL_CONTROLLER_AXIS_TRIGGERLEFT, nullptr);
	if (trigger > joy_maxtrigl[port]) {
		if (! joy_trigl_on[port]) {
			if (GetPlayMode() == PlayMode::Play) SetPlayMode(PlayMode::Rewind);
			joy_trigl_on[port] = true;
		}
		joy_maxtrigl[port] = +(JOYSTICK_DEAD_ZONE - JOYSTICK_HYSTERESIS);
	} else {
		if (joy_trigl_on[port]) {
			if (GetPlayMode() == PlayMode::Rewind) SetPlayMode(PlayMode::Play);
	...
	trigger = get_controller_axis(port, SDL_CONTROLLER_AXIS_TRIGGERRIGHT, nullptr);
	if (trigger > joy_maxtrigr[port]) {
		if (! joy_trigr_on[port]) {
			if (GetPlayMode() == PlayMode::Play) SetPlayMode(PlayMode::Forward);
			joy_trigr_on[port] = true;
		}
		joy_maxtrigr[port] = +(JOYSTICK_DEAD_ZONE - JOYSTICK_HYSTERESIS);
	} else {
		if (joy_trigr_on[port]) {
			if (GetPlayMode() == PlayMode::Forward) SetPlayMode(PlayMode::Play);
	...
	int x = get_controller_axis(port, SDL_CONTROLLER_AXIS_LEFTX, nullptr);
	int y = get_controller_axis(port, SDL_CONTROLLER_AXIS_LEFTY, nullptr);

	// In twin-stick mode, axes from right stick on opposite port
	if (ThePrefs.TwinStick) {
		x = get_controller_axis(port ^ 1, SDL_CONTROLLER_AXIS_RIGHTX, nullptr);
		y = get_controller_axis(port ^ 1, SDL_CONTROLLER_AXIS_RIGHTY, nullptr);
	}
