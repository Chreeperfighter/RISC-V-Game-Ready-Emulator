...
int Frodo::ReadyToRun()
{
	// Load preferences
	if (prefs_path.empty()) {
		prefs_path = "/frodo/config";
		snapshot_path = "/frodo/snapshots";
		mkdir("/frodo", 0755);
		mkdir("/frodo/snapshots", 0755);
	}
	...
	// Save preferences
	if (! ThePrefs.Save(prefs_path)) {
		fprintf(stderr, "WARNING: Could not save preferences to '%s'\n", prefs_path.c_str());
	}
...
{
	// No prefs editor UI on RV32 — ignore F10 and keep running
	return true;
}
